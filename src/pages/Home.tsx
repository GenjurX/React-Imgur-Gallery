...
const Home: React.FC<IHome> = (props) => {
  ...
  const handleSearch = async () => {
    showLoading();
    try {
      const res = await axios(`https://api.imgur.com/3/gallery/${(filter.section !== 'select') ? `/${filter.section}` : ''}${(filter.sort !== 'select') ? `/${filter.sort}` : ''}${(filter.window !== 'select') ? `/${filter.window}` : ''}?showViral=${filter.enViral}`, {
        headers: {
  ...
  return (
    <div >
      ...
      <div className='mt-48 px-56'>
        ...
        >
          <div className='grid grid-cols-3 gap-4'>
            {items && items.map(({ link, title, ...item }: IImage, index: number) => (
              <div key={index} className='p-2 cursor-pointer rounded overflow-hidden shadow-md hover:bg-blue-100' onClick={() => onClickHandler({ link, title, ...item })}>
                <img className='object-cover h-48 w-full' src={link} alt='Imgur' />
                <div className='truncate flex justify-center font-bold my-2'>{title ? title : 'No description'}</div>
              </div>
            ))}
          </div>
        </InfiniteScroll>
